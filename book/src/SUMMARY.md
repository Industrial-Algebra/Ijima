# Summary

- [Introduction](./introduction.md)
- [Getting Started](./getting-started.md)

# Concepts

- [The Two-Store Model](./concepts/two-store-model.md)
- [Namespaces & Multi-Tenancy](./concepts/multi-tenancy.md)
- [Provenance & Trust Tiers](./concepts/provenance.md)
- [Capabilities & GrantTokens](./concepts/capabilities.md)
- [Sessions & the Miner](./concepts/mining.md)

# Guide

- [Installing and Configuring](./guide/installation.md)
- [Running the Daemon](./guide/daemon.md)
- [Token Management](./guide/tokens.md)
- [Importing Legacy Corpora](./guide/import.md)
- [The Mining Pipeline](./guide/mining.md)
- [pi Thin-Client Integration](./guide/pi.md)
- [Deploying on a Server](./guide/deploy.md)
- [Feature Flags](./guide/feature-flags.md)

# API Reference

- [HTTP API Overview](./api/overview.md)
- [The Client Crate](./api/client.md)
- [CLI Reference](./api/cli.md)

# Design

- [Security Considerations](./design/security.md)
- [Architecture](./design/architecture.md)
- [Decision Log (ADRs)](./design/decisions.md)
- [Roadmap](./design/roadmap.md)

# Examples

- [Basic Usage](./examples/basic.md)
- [Multi-Source Import](./examples/import.md)
