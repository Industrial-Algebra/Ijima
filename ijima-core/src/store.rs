...
pub trait Store: Send + Sync {
    ...
    /// design: Explicit, Doctrine, Mined, and imported rows are never
    /// eligible; only ambient auto-capture chatter ages out. Default:
    /// no-op (backends without lifecycle support).
    async fn delete_aged_autocapture(&self, _cutoff_epoch_secs: &str) -> Result<u64> {
        Ok(0)
    }
